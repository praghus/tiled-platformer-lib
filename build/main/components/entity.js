...
class Entity {
    ...
    update(scene, delta = 0) {
        ...
        if (helpers_1.isValidArray(this.collisionLayers) && this.collisionMask) {
            for (const layerId of this.collisionLayers) {
                ...
                if (layer.type === enums_1.NODE_TYPE.LAYER) {
                    for (let y = PY; y < PH; y++) {
                        for (let x = PX; x < PW; x++) {
    ...
    _displayDebug(ctx, scene) {
        ...
        ctx.lineWidth = 0.1;
        helpers_1.outline(ctx)(posX, posY, width, height, visible ? enums_1.COLORS.WHITE_30 : enums_1.COLORS.PURPLE);
        ctx.lineWidth = 0.5;
        const color = this.collided.length ? enums_1.COLORS.LIGHT_RED : enums_1.COLORS.GREEN;
        helpers_1.stroke(ctx)(posX, posY, collisionMask.points, visible ? color : enums_1.COLORS.PURPLE);
        const text = helpers_1.fillText(ctx);
        const [x, y] = [posX + width + 4, posY + height / 2];
        text(`${type}`, posX, posY - 10, enums_1.COLORS.WHITE);
