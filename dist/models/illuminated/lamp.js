...
var Lamp = function (_Light) {
    ...
    _createClass(Lamp, [{
        ...
        value: function center() {
            return new _vec2.default((1 - Math.cos(this.angle) * this.roughness) * this.distance, (1 + Math.sin(this.angle) * this.roughness) * this.distance);
        }
    }, {
        key: 'bounds',
        value: function bounds() {
            var orientationCenter = new _vec2.default(Math.cos(this.angle), -Math.sin(this.angle)).mul(this.roughness * this.distance);
            return {
                topleft: new _vec2.default(this.position.x + orientationCenter.x - this.distance, this.position.y + orientationCenter.y - this.distance),
                bottomright: new _vec2.default(this.position.x + orientationCenter.x + this.distance, this.position.y + orientationCenter.y + this.distance)
            };
        }
    }, {
        key: 'mask',
        ...
        value: function forEachSample(f) {
            ...
            for (var s = 0; s < this.samples; ++s) {
                ...
                var delta = new _vec2.default(Math.cos(a) * r, Math.sin(a) * r);
