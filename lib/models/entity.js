...
export default class Entity {
    ...
    move () {
        ...
        const PX = Math.floor((this.x) / spriteSize)
        const PY = Math.floor((this.y) / spriteSize)
        const PW = Math.floor((this.expectedX + this.width) / spriteSize)
        const PH = Math.floor((this.expectedY + this.height) / spriteSize)

        isValidArray(this.collisionLayers) && this.collisionLayers.map(
            (layer) => {
                for (let y = PY; y <= PH; y++) {
                    for (let x = PX; x <= PW; x++) {
                        ...
                        if (world.isSolidTile(t)) {
                            ...
                            const ccY = td.collide(
                                this.getTranslatedBounds(this.x - tileX, this.expectedY - tileY)
                            )
                            const ccX = td.collide(
                                this.getTranslatedBounds(this.expectedX - tileX, this.y - tileY)
                            )
                            if (ccX) {
                                if (Math.abs(ccX.overlapV.y) && !(isOneWay && this.force.y < 0 && !this.onFloor)) {
                            ...
                            if (ccY && this.force.y !== 0 && !(isOneWay && this.force.y < 0 && !this.onFloor)) {
