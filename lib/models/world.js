...
import { DarkMask, DiscObject, Vec2, Lighting } from './illuminated'
import { createPolygonObject, createDiscObject, isValidArray, getFilename } from '../helpers'
    ...
    update () {
        ...
        this.getObjectLayers().map((layer) => {
            layer.objects.map((obj) => {
                if (!obj.dead) {
                    if(obj.type === 'item' && obj.properties.id === 'chopper'){
                        console.info(obj)
                    }
                    if (obj.onScreen() || obj.activated) {
                        ...
                        if (obj.light) {
                            obj.light.position = new Vec2(
                                obj.x + (obj.width / 2) + camera.x,
                                obj.y + (obj.height / 2) + camera.y
                            )
                            this.lights.push(obj.light)
                        }
                        if (obj.shadowCaster) {
                            this.lightmask.push(
                                createDiscObject(obj.x + camera.x, obj.y + camera.y, obj.width / 2)
                            )
                        }
