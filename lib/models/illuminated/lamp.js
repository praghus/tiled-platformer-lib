...
export default class Lamp extends Light {
    ...
    _getGradientCache (center) {
        ...
        const D = d * 2
        const cache = createCanvasAnd2dContext(D, D)
    ...
    bounds () {
        const orientationCenter = new Vector(Math.cos(this.angle), -Math.sin(this.angle)).mul(this.roughness * this.distance)
        return {
            topleft: new Vector(this.position.x + orientationCenter.x - this.distance, this.position.y + orientationCenter.y - this.distance),
            bottomright: new Vector(this.position.x + orientationCenter.x + this.distance, this.position.y + orientationCenter.y + this.distance)
        }
    }
